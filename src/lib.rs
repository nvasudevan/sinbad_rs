...
use sinbad::SinBAD;
use std::env;
use crate::sinbad::{SinBADError, SinBADInput, SinBADOutput};
...
pub fn sinbad() -> Result<SinBAD, SinBADError> {
    let timeout_cmd = env::var("TIMEOUT_CMD")
        .map_err(|_|
            SinBADError::new(
                Some(-1),
                "Environment variable TIMEOUT_CMD is not set".to_owned(),
            )
        )?;
    let sinbad_cmd = env::var("SINBAD_CMD")
        .map_err(|_|
            SinBADError::new(
                Some(-1),
                "Environment variable SINBAD_CMD is not set".to_owned(),
            )
        )?;
    let accent_dir = env::var("ACCENT_DIR")
        .map_err(|_|
            SinBADError::new(
                Some(-1),
                "Environment variable ACCENT_DIR is not set".to_owned(),
            )
        )?;
...
mod tests {
    ...
    fn test_sinbad() {
        let sin_input = SinBADInput::new("dynamic1", 10, 5);
